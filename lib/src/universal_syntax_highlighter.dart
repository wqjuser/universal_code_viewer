...
class UniversalSyntaxHighlighter {
  static final Map<String, RegExp> languagePatterns = {
    ...
    'react': RegExp(
        r'(import\s+.*from|export\s+default|class\s+\w+|function\s+\w+)'),
    'vue':
        RegExp(r'(import\s+.*from|export\s+default|<template|<script|<style)'),
    'angular':
        RegExp(r'(import\s+.*from|export\s+default|@Component|@NgModule)'),
    'django':
        RegExp(r'(from\s+django|from\s+rest_framework|class\s+\w+|def\s+\w+)'),
    'flask':
        RegExp(r'(from\s+flask|from\s+flask_restful|class\s+\w+|def\s+\w+)'),
    'nodejs':
        RegExp(r'(const|let|var|function|import\s+.*from|module.exports)'),
    'express':
        RegExp(r'(const|let|var|function|import\s+.*from|app\..*|router\..*)'),
...
// VB.NET
  ...
  void _normalizeSpans() {
    ...
    _spans.removeWhere((span) =>
        span.start >= span.end || span.start < 0 || span.end > source.length);
